...
| `GET /api/schedule?date=YYYY-MM-DD&sport=SHO` | India's sessions for a competition day (JST calendar). Defaults to today |
| `GET /api/live`                               | Sessions in progress, each with its current scoreboard                   |
